...
export default class AnnualReportService {
    static async getAnnualReportsFromDanishBusinessRegistrationAPI(cvrNumber) {
        ...
        const annualReports = await Promise.all(financialStatements.map(async (report) => {
            const documents = await Promise.all(report.dokumenter
    ...
    static extractAnnualReportFromXML(xml) {
        ...
        const acceptedTaxonomies = ÅRL_TAXONOMY.schema;
        if (!acceptedTaxonomies.some((t) => taxonomy.includes(t))) {
            console.warn(`The taxonomy ${taxonomy} is not accepted. Accepted taxonomies are: ${acceptedTaxonomies.join(", ")}`);
            return null;
        }
        const annualReport = xbrlDocument.extractTaxonomyData();
        return annualReport;
    }
    static async getAnnualReports(cvrNumber) {
        ...
        const annualReports = xmlAnnualReports
            ...
            const documents = xmlAnnualReport.documents
                ...
                if (!annualReport) {
                    console.warn(`No valid annual report found for CVR number ${xmlAnnualReport.cvrNumber} in document ${document.dokumentUrl}`);
                    return null;
                }
